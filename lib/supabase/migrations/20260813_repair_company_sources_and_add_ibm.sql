-- Repair replacement projects where the monitoring tables were created but
-- the additive job_sources columns from the monitoring migration were absent.
...
DROP POLICY IF EXISTS "Users can view own source subscriptions" ON user_job_source_subscriptions;
CREATE POLICY "Users can view own source subscriptions"
  ON user_job_source_subscriptions FOR SELECT USING (auth.uid() = user_id);
DROP POLICY IF EXISTS "Users can insert own source subscriptions" ON user_job_source_subscriptions;
...
DROP POLICY IF EXISTS "Users can update own source subscriptions" ON user_job_source_subscriptions;
...
DROP POLICY IF EXISTS "Users can delete own source subscriptions" ON user_job_source_subscriptions;
...
DROP POLICY IF EXISTS "Users can view own alert candidates" ON discovery_alert_candidates;
CREATE POLICY "Users can view own alert candidates"
  ON discovery_alert_candidates FOR SELECT USING (auth.uid() = user_id);
DROP POLICY IF EXISTS "Users can view own discovery notifications" ON discovery_notifications;
CREATE POLICY "Users can view own discovery notifications"
  ON discovery_notifications FOR SELECT USING (auth.uid() = user_id);
DROP POLICY IF EXISTS "Users can update own discovery notifications" ON discovery_notifications;
CREATE POLICY "Users can update own discovery notifications"
  ON discovery_notifications FOR UPDATE USING (auth.uid() = user_id);

DROP TRIGGER IF EXISTS update_source_subscriptions_updated_at ON user_job_source_subscriptions;
...
-- IBM serves jobs through its public IBM Search API rather than embedding
-- JobPosting JSON-LD in the careers landing page.
...
-- Make newly added columns immediately available through Supabase's REST API.
